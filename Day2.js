...
function simplifyInput(){
    return input = input.map(x=>x.split(' ')).map(x=>[x[0],parseInt(x[1])]);
}
...
function subDepth(amount){
    return y-=amount;
}

function addHorizontal(amount){
    return x+=amount;
}
...
function getFinalResult(){
    ...
    for(let i = 0; i < input.length; i++){
        if(input[i][0] == 'forward'){
            addHorizontal(input[i][1]);
        }else if(input[i][0] == 'down'){
            addDepth(input[i][1]);
...
function addAim(amount){
    return aim+=amount;
}

function subAim(amount){
    return aim-=amount;
}

function getPart2FinalResult(){
    simplifyInput();
    for(let i = 0; i < input.length; i++){
        if(input[i][0] == 'forward'){
            addHorizontal(input[i][1]);
            newAddDepth(input[i][1]);
        }else if(input[i][0] == 'down'){
            addAim(input[i][1]);
        }else if(input[i][0] == 'up'){
            subAim(input[i][1]);
